...
TEST_CASE( "LRUC is testing", "[LRUC]" )
{
    ...
    SECTION( "Making same requests" )
    {
        ...
        {
            if (lruc.lookup_update(1, slow_get_page_int) )
                hits ++;
        }
    ...
    SECTION( "Making complitely different requests" )
    {
        for (int i = 0; i < NUMBER_OF_REQUESTS; i ++)
        {
            if (lruc.lookup_update(i, slow_get_page_int) )
                hits ++;
        }
    ...
    SECTION( "Making random requests" )
    {
        ...
        for (int i = 0; i < NUMBER_OF_REQUESTS; i ++)
        {
            if (lruc.lookup_update( distrib(random), slow_get_page_int) )
                hits ++;
        }
